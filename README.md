...
非アクティブモードの付箋のテキスト表示エリアを右クリックすると、コンテキストメニューが表示されます：

- **アクティブ時の座標・サイズを初期化**: 付箋がアクティブになった時の表示位置とサイズを現在の非アクティブ位置にリセット
  - 付箋の表示位置が画面外に移動してしまった場合の復旧に便利
  - アクティブ時のサイズが大きくなりすぎた場合の調整に使用
...
- **ダブルクリック**: アクティブモード時、URL部分をダブルクリックでブラウザ表示
- **対応URL**: http://、https://で始まるURLに対応
- **非表示時**: 非アクティブモードでは通常テキストとして表示
...
アクティブモードの付箋に対してのみ使用できるショートカットキーを設定できます：

- **アクティブ付箋のピン留め切り替え**: 現在編集中の付箋のピン留めを切り替え
- **アクティブ付箋のロック切り替え**: 現在編集中の付箋のアクティブロックを切り替え
- ホットキーとは別に設定可能で、アクティブ付箋に対してのみ機能
...
**📏 サイズ設定**
- **非アクティブモードの幅**: 付箋の非アクティブ時の幅を設定（50-300px）
- **非アクティブモードの高さ**: 付箋の非アクティブ時の高さを設定（50-200px）
- **非アクティブモードのフォントサイズ**: 非アクティブ時のフォントサイズを設定（8-20px）
...
### v1.1.1 (2024年7月)
- **ESCキー操作の改善**: ESC２回連続→１回押しでアクティブモード終了に変更
- **状態不整合の修正**: アイコンが非アクティブ状態でもウィンドウサイズが戻らない問題を解決
